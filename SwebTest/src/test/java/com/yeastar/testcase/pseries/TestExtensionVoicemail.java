...
public class TestExtensionVoicemail extends TestCaseBase {
    ...
    private final String reqDataCreateExtension_new = String.format("" +
    ...
    public void preparationSteps() {
        ...
        if(runRecoveryEnvFlag){
            ...
            apiUtil.createExtension(reqDataCreateExtension.replace("GROUPLIST",groupList))
                    .createExtension(reqDataCreateExtension_new.replace("EXTENSIONNUM", "1").replace("EXTENSIONLASTNAME", "").replace("GROUPLIST", groupList))
                    .apply();
            ...
            apiUtil.loginWebClient("1", EXTENSION_PASSWORD, EXTENSION_PASSWORD_NEW);
            apiUtil.apply();

            auto.loginPage().login("1",EXTENSION_PASSWORD_NEW);
            ...
            pjsip.Pj_CreateAccount(1,EXTENSION_PASSWORD,"UDP",UDP_PORT,-1);
            pjsip.Pj_Register_Account_WithoutAssist_For_PSeries(0,DEVICE_IP_LAN);
            pjsip.Pj_Register_Account_WithoutAssist_For_PSeries(1,DEVICE_IP_LAN);
            auto.extensionPage().editExtension(getDriver(),"0").recordVoicemailGreeting("0-Yeastar Test0 朗视信息科技","test");
            pjsip.Pj_Make_Call_No_Answer(1, "0" , DEVICE_IP_LAN, false);
            log.debug("[extension status]--->"+getExtensionStatus(0, RING, 8));
...
//        pjsip.Pj_Init();
        ...
        sleep(2000);
        pjsip.Pj_Send_Dtmf(0,"404");
        sleep(30000);
        pjsip.Pj_Hangup_All();

        assertStep("cli确认播放提示音为vm-greeting-leave-after-tone.slin");
        String result = SSHLinuxUntils.exeCommand(DEVICE_IP_LAN, PJSIP_TCP_PORT, PJSIP_SSH_USER, PJSIP_SSH_PASSWORD, SHOW_CLI_LOG);
        log.debug("[ssh result] "+result);
        softAssert.assertTrue(result.contains("vm-greeting-leave-after-tone.slin"),"[Assert,cli确认voicemail提示音]");
