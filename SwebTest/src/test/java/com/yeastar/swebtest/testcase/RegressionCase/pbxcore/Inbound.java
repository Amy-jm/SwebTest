...
//        Holiday通话验证
    ...
    public void F_timecondition4() throws InterruptedException {
        Reporter.infoExec(" 分机1000拨打特征码*801强制启用工作时间"); //执行操作
        pjsip.Pj_Make_Call_Auto_Answer(1000,"*801",DEVICE_IP_LAN);
        ys_waitingTime(5000);
        pjsip.Pj_Hangup_All();
...
//    删除操作
    ...
    public void G_delete1() throws InterruptedException {
        ...
        Reporter.infoExec(" 删除：DeleteTest-取消删除"); //执行操作
        int row8 = Integer.parseInt(String.valueOf(gridLineNum(inboundRoutes.grid)));
        int row4 = Integer.parseInt(String.valueOf(gridFindRowByColumn(inboundRoutes.grid,inboundRoutes.gridcolumn_Name,"DeleteTest",sort_ascendingOrder)));
        ...
        YsAssert.assertEquals(row8,row5,"删除：DeleteTest-取消删除");

        Reporter.infoExec(" 删除：DeleteTest-确定删除"); //执行操作
        inboundRoutes.delete.click();
        inboundRoutes.delete_yes.click();
        ys_waitingLoading(inboundRoutes.grid_Mask);
        ...
        YsAssert.assertEquals(row5,row7,"删除：DeleteTest-确定删除");
        ys_apply();
